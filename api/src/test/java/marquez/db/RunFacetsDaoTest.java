...
public class RunFacetsDaoTest {
  ...
  @Test
  public void testInsertRunFacetsForNominalTime() {
  ...
  @Test
  public void testInsertRunFacetsForParent() {
    RunFacetsDao.RunFacetRow row = getRunFacetRow("parent").get(0);
    ...
    assertThat(row.facet().toString())
        .startsWith(
            String.format(
                "{\"parent\": {\"job\": {\"name\": \"name\", \"namespace\": \"namespace\"}, \"run\": "
                    + "{\"runId\": \"%s\"}, \"_producer\": \"http://test.producer/\", "
                    + "\"_schemaURL\": \"http://test.schema/\"}}",
                lineageRow.getRun().getParentRunUuid().get()));
  }

  @Test
  public void testInsertRunFacetsForErrorMessage() {
    RunFacetsDao.RunFacetRow row = getRunFacetRow("errorMessage").get(0);

    assertThat(row.lineageEventTime()).isEqualTo(lineageRow.getJob().getCreatedAt());
    assertThat(row.lineageEventType()).isEqualTo("COMPLETE");
    assertThat(row.facet().toString())
        .isEqualTo("{\"errorMessage\": \"{some-error-message-facet:true}\"}");
  }
