...
public interface LineageDao {
  @SqlQuery(
      "select distinct io_in_2.job_uuid\n"
          + "from jobs j\n"
          + "inner join job_versions_io_mapping_input io_in on io_in.job_version_uuid = j.current_version_uuid\n"
          + "inner join job_versions_io_mapping_input io_in_2 on io_in.dataset_uuid = io_in_2.dataset_uuid\n"
          + "where j.uuid in (<jobIds>)\n"
          + "UNION \n"
          + "select distinct io_out_2.job_uuid\n"
          + "from jobs j\n"
          + "inner join job_versions_io_mapping_output io_out on io_out.job_version_uuid = j.current_version_uuid\n"
          + "inner join job_versions_io_mapping_output io_out_2 on io_out_2.dataset_uuid = io_out.dataset_uuid\n"
          + "where j.uuid in (<jobIds>)\n"
          + "UNION \n"
          + "select distinct io_out.job_uuid\n"
          + "from jobs j\n"
          + "inner join job_versions_io_mapping_input io_in on io_in.job_version_uuid = j.current_version_uuid \n"
          + "inner join job_versions_io_mapping_output io_out on io_out.dataset_uuid = io_in.dataset_uuid \n"
          + "where j.uuid in (<jobIds>)\n"
          + "UNION \n"
          + "select distinct io_in.job_uuid\n"
          + "from jobs j\n"
          + "inner join job_versions_io_mapping_output io_out on io_out.job_version_uuid = j.current_version_uuid\n"
          + "inner join job_versions_io_mapping_input io_in on io_in.dataset_uuid = io_out.dataset_uuid\n"
          + "where j.uuid in (<jobIds>)")
