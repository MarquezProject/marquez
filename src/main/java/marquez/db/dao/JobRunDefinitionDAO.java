...
public interface JobRunDefinitionDAO {
  @SqlQuery("SELECT jrd.guid, jrd.job_version_guid, jrd.run_args_json, jrd.nominal_time, j.name, j.current_ownership, jv.uri FROM job_run_definitions jrd INNER JOIN job_versions jv ON (jv.guid = jrd.job_version_guid) INNER JOIN jobs j ON(j.guid = jv.job_guid) WHERE jrd.guid = :guid")
