...
public interface JobRunDefinitionDAO {
  ...
  @SqlQuery("SELECT * FROM job_run_definitions WHERE hash = :hash")
  JobRunDefinition findByHash(@Bind("hash") UUID hash);

  @SqlUpdate(
      "INSERT INTO job_run_definitions (guid, hash, job_version_guid, run_args_json) VALUES (:guid, :hash, :job_version_guid, :run_args)")
  void insert(
