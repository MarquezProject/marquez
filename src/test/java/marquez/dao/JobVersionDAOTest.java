...
public class JobVersionDAOTest {
  ...
  final Job job = new Job(UUID.randomUUID(), "a job", "an owner", new Timestamp(new Date(0).getTime()), "category", "description", "http://foo.com", nsID);
  ...
  public void testFindByVersion() {
    ...
    jobVersionDAO.insert(jobVersion.getGuid(), jobVersion.getVersion(), jobVersion.getJobGuid(), jobVersion.getURI());
  ...
  public void testFindByVersion_Multi() {
    ...
    jobVersionDAO.insert(jobVersion1.getGuid(), jobVersion1.getVersion(), jobVersion1.getJobGuid(), jobVersion1.getURI());
    jobVersionDAO.insert(jobVersion2.getGuid(), jobVersion2.getVersion(), jobVersion2.getJobGuid(), jobVersion2.getURI());
  ...
  public void testFind() {
    ...
    versionsWeWant.forEach(jv -> jobVersionDAO.insert(jv.getGuid(), jv.getVersion(), jv.getJobGuid(), jv.getURI()));
  ...
  public void testFind_Multi() {
    ...
    JobVersion dontWant1 = new JobVersion(UUID.randomUUID(), unrelatedJob.getGuid(), "http://random.version", UUID.randomUUID());
    JobVersion dontWant2 = new JobVersion(UUID.randomUUID(), unrelatedJob.getGuid(), "http://random.version", UUID.randomUUID());
    List<JobVersion> versionsWeDontWant = Arrays.asList(dontWant1, dontWant2);
    versionsWeDontWant.forEach(jv -> jobVersionDAO.insert(jv.getGuid(), jv.getVersion(), jv.getJobGuid(), jv.getURI()));
