...
public class JobRunIntegrationTest {
  ...
  static final String JOB_RUN_TEST_PREFIX = "job_run_test_";
    ...
    daoSetup
        ...
        .useHandle(
            handle -> {
              handle.execute(
                  format(
                      "merge into jobs (guid, name, category, description) values ('%s', 'my_job', 'testing', 'fake job for reference');",
                      TEST_JOB_GUID));
              handle.execute(
                  format(
                      "merge into job_versions (guid, input_dataset, output_dataset, job_guid) values ('%s', 'input_set1', 'output_set1', '%s');",
                      TEST_JOB_RUN_VERSION_GUID, TEST_JOB_GUID));
              handle.execute(
                  format(
                      "merge into job_run_definitions (guid, job_version_guid, run_args_json, nominal_time) values ('%s', '%s', '--my-favorite-flag', '2018-09-06T05:24:50+00:00');",
