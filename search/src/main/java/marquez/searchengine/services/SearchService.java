...
public class SearchService {
  ...
  public SearchService(OpenLineageDao openLineageDao) throws IOException {
    ...
    this.datasetIndexWriter =
        new IndexWriter(datasetIndexDirectory, new IndexWriterConfig(analyzer));
  ...
  private void indexLineageEvents(@Valid @NotNull List<LineageEvent> lineageEvents)
      throws IOException {
    // for dedup purpose
      ...
      if (event.getInputs() != null) {
        for (Dataset input : event.getInputs()) {
          Map<String, Object> inputMap =
              mapDatasetEvent(input, event.getRun().getRunId(), event.getEventType());
      ...
      if (event.getOutputs() != null) {
        for (Dataset output : event.getOutputs()) {
          Map<String, Object> outputMap =
              mapDatasetEvent(output, event.getRun().getRunId(), event.getEventType());
  ...
  private Map<String, Object> mapJobEvent(LineageEvent event) {
    ...
    Optional.ofNullable(event.getRun().getFacets())
        .ifPresent(facets -> jobMap.put("facets", facets));
    return jobMap;
  }

  private boolean documentAlreadyExists(String uniqueId, Directory indexDirectory)
      throws IOException {
  ...
  public void indexJobDocuments(List<Map<String, Object>> documents, IndexWriter writer)
      throws IOException {
  ...
  public void indexDatasetDocuments(List<Map<String, Object>> documents, IndexWriter writer)
      throws IOException {
    for (Map<String, Object> document : documents) {
      if (documentAlreadyExists((String) document.get("id"), datasetIndexDirectory)) {
        continue;
  ...
  private Document createDatasetDocument(Map<String, Object> document) {
    ...
    if (document.containsKey("inputFacets")) {
      doc.add(
          new TextField("inputFacets", document.get("inputFacets").toString(), Field.Store.YES));
    }
    if (document.containsKey("outputFacets")) {
      doc.add(
          new TextField("outputFacets", document.get("outputFacets").toString(), Field.Store.YES));
    }
    return doc;
  }
  ...
  private IndexResponse createIndexResponse(String index, String id, boolean created) {
    ...
    IndexResponse.ShardInfo shardInfo =
        new IndexResponse.ShardInfo(1, 1, 0); // 1 shard, 1 successful, 0 failed
  ...
  private SearchResult search(String query, List<String> fields, Directory indexDirectory)
      throws Exception {
    ...
    try (DirectoryReader reader = DirectoryReader.open(indexDirectory)) {
      ...
      MultiFieldQueryParser parser =
          new MultiFieldQueryParser(fields.toArray(new String[0]), analyzer);
      ...
      for (int i = 0; i < topDocs.scoreDocs.length; i++) {
        ...
        result.addDocument(
            indexDirectory == jobIndexDirectory ? "jobs" : "datasets", allDoc, highlight, i);
      }
