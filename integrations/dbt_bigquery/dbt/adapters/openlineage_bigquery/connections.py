...
PRODUCER = f"dbt-openlineage_bigquery/{VERSION}"
...
class OpenLineageBigQueryConnectionManager(BigQueryConnectionManager):
    ...
    def emit_complete(self, run_id):
        ...
        self.get_openlineage_client().emit(RunEvent(
            ...
            job=Job(namespace=meta.namespace, name=meta.name),
            producer=PRODUCER,
            inputs=[input.to_openlineage_dataset(meta.namespace) for input in inputs],
            outputs=[output.to_openlineage_dataset(meta.namespace)]
        ))
        logger.debug(f'openlineage complete event with run_id: {run_id}')
